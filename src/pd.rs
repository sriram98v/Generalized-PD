...
impl<'a> PhylogeneticDiversity for TreePDMap<'a>{
    ...
    fn compute_norm_min(&self)->(Vec<Vec<(f32, u32)>>, Vec<Vec<Vec<usize>>>,Vec<Vec<(f32, u32)>>, Vec<Vec<Vec<usize>>>)
    {
        let num_leaves = self.tree.get_leaves().len();
        let mut delta_bar = vec![vec![(f32::INFINITY, 0_u32);num_leaves+1];self.tree.get_nodes().len()];
        let mut delta_bar_sets: Vec<Vec<Vec<usize>>> = vec![vec![vec![];num_leaves+1];self.tree.get_nodes().len()];
        let mut delta_hat = vec![vec![(f32::INFINITY, 0_u32);num_leaves+1];self.tree.get_nodes().len()];
        let mut delta_hat_sets: Vec<Vec<Vec<usize>>> = vec![vec![vec![];num_leaves+1];self.tree.get_nodes().len()];
            ...
            if !node.is_leaf(){
                for i in 1..std::cmp::min(num_leaves, self.tree.get_cluster_size(node.get_id()))+1{
                    ...
                    for r in 0..i+1{
                        ...
                        let val_bar = delta_bar[y][r].0 + (self.tree.get_node(y).unwrap().get_weight().unwrap()*(std::cmp::min(r,1) as f32)) 
                            + delta_bar[x][l].0 + (self.tree.get_node(x).unwrap().get_weight().unwrap()*(std::cmp::min(l,1) as f32));
                        let mut e = 0_u32;
                        let mut set = vec![];
                        if l==0{
                            e += delta_bar[y][r].1 + 1;
    ...
    fn get_minPD(&self, num_taxa: usize)-><<<Self as PhylogeneticDiversity>::Tree as RootedTree>::Node as RootedWeightedNode>::Weight
    {
        
        self.precomputed_min[self.tree.get_root_id()][std::cmp::min(num_taxa, self.tree.num_taxa())].0
    }

    fn get_minPD_node(&self, node_id: <<Self as PhylogeneticDiversity>::Tree as RootedTree>::NodeID, num_taxa: usize)-><<<Self as PhylogeneticDiversity>::Tree as RootedTree>::Node as RootedWeightedNode>::Weight
    {    
        self.precomputed_min[node_id][std::cmp::min(num_taxa, self.tree.num_taxa())].0
    }


    fn get_norm_minPD(&self, num_taxa: usize)-><<<Self as PhylogeneticDiversity>::Tree as RootedTree>::Node as RootedWeightedNode>::Weight
    {
        self.precomputed_norm_min[self.tree.get_root_id()][std::cmp::min(num_taxa, self.tree.num_taxa())].0
    }

    fn backtrack_min(&self, node_id: <<Self as PhylogeneticDiversity>::Tree as RootedTree>::NodeID, num_taxa: usize, taxaset: &mut Vec<<<Self as PhylogeneticDiversity>::Tree as RootedTree>::NodeID>)
    {
        if self.tree.is_leaf(&node_id){
            ...
            if self.get_minPD_node(node_id, num_taxa)==self.get_minPD_node(left_child_id, num_taxa)+left_edge_weight
            {
            ...
            else if self.get_minPD_node(node_id, num_taxa)==self.get_minPD_node(right_child_id, num_taxa)+right_edge_weight
            {
                ...
                let ((left_pd, left_edges),(right_pd, right_edges)) = (1..num_taxa).map(|l| (l, num_taxa-l))
                    .map(|(l,r)| (self.precomputed_min[left_child_id][std::cmp::min(l, self.tree.num_taxa())], self.precomputed_min[right_child_id][std::cmp::min(r, self.tree.num_taxa())]))
                    .filter(|(l,r)| l.0!=f32::INFINITY && r.0!=f32::INFINITY )
                    // .inspect(|x| println!("{:?}", &x))
                    .min_by(|x,y| (x.0.0+x.1.0).total_cmp(&(y.0.0+y.1.0))).unwrap();
    ...
    fn get_minPD_taxa_set(&self, num_taxa: usize)->impl Iterator<Item = <<Self as PhylogeneticDiversity>::Tree as RootedTree>::NodeID>
    {
        self.precomputed_min_set[self.tree.get_root_id()][num_taxa].clone().into_iter()
    }

    fn get_norm_minPD_taxa_set(&self, num_taxa: usize)->impl Iterator<Item = <<Self as PhylogeneticDiversity>::Tree as RootedTree>::NodeID>
    {
        self.precomputed_norm_min_set[self.tree.get_root_id()][num_taxa].clone().into_iter()
